...
parser.add_argument('--noise-rate', default=0.0, type=float, help='noisy rate')
parser.add_argument('--gnum', default=0, type=int, help='group num')
...
if dataset == 'sysu':
    ...
    if len(args.resume_net1) > 0:
        model_path1 = os.path.join(checkpoint_path,args.resume_net1)
        model_path2 = os.path.join(checkpoint_path,args.resume_net2)
    ...
    for trial in range(10):
        test_trial = trial + 1
        model_path1 = os.path.join(checkpoint_path, (args.resume_net1).format(test_trial))
        model_path2 = os.path.join(checkpoint_path, (args.resume_net2).format(test_trial))
        if os.path.isfile(model_path1):
...
print('Rank-1: {:.2%} | Rank-5: {:.2%} | Rank-10: {:.2%}| Rank-20: {:.2%}| mAP: {:.2%}| mINP: {:.2%}'.format(
    cmc[0], cmc[4], cmc[9], cmc[19], mAP, mINP))
wandb.log({"Rank-1": cmc[0], "Rank-5": cmc[4], "Rank-10": cmc[9], "mAP": mAP, "mINP": mINP})
