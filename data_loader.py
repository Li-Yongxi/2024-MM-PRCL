...
class SYSUData(data.Dataset):
    def __init__(self, data_dir, transform=None, colorIndex=None, thermalIndex=None, noise_rate=0., noise_file='', mode='', probV=[], probI=[]):
        ...
        self.noisy_pseudo_label_color = -1. * torch.ones(len(self.train_color_label))
        self.noisy_pseudo_label_thermal = -1. * torch.ones(len(self.train_thermal_label))
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'warmup':
            ...
            return (img1_0, img1_1, img2, target1, target2, true_target1, true_target2, probV_1, probV_2, probI
                    , self.cIndex[index], self.tIndex[index])
...
class RegDBData(data.Dataset):
    def __init__(self, data_dir, trial, transform=None, colorIndex=None, thermalIndex=None, noise_rate=0.,
        ...
        self.train_color_label = np.array(self.train_color_label)
        self.train_thermal_label = np.array(self.train_thermal_label)
        ...
        if noise_rate == 0.:
            ...
            self.ir_cleanIdx = range(len(self.train_thermal_label))
            self.ir_noiseIdx = []
            self.true_train_color_label = np.array(self.train_color_label)
            self.true_train_thermal_label = np.array(self.train_thermal_label)
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'warmup':
            ...
            probV_1, probV_2, probI = self.probV_1[self.cIndex[index]], self.probV_2[self.cIndex[index]], self.probI[self.tIndex[index]]
            return img1_0, img1_1, img2, target1, target2, true_target1, true_target2, probV_1, probV_2, probI, self.cIndex[index], self.tIndex[index]
